...
user function funci2()
local oBrowse :=FWMBrowse():new()// criação do browse


oBrowse:SetAlias('ZA1') 
oBrowse:SetMenuDef('ios2') // define setmenudef
...
return FWMVCMenu('ios2') //menu padrão c.r.u.d.
...
If ZA0->(DBSeek(xFilial("ZA0"or )) + cCodAut) // se XFILIAL ZA0 + codigo do autor(cCodAutor) == verdadeiro 

resut:=ZA0->ZA0_DTAFAL // armazena data de falecimento em (resut)
// 2) pega a data de falecimento
 If empty(resut) == .T. // se a variavel resut estiver vazia(empty) entao...
